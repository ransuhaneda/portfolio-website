...
describe('blog content contract', () => {
  ...
  it('generates published-only outputs and removes only owned renamed/deleted mirrors', async () => {
    ...
    try {
      ...
      run()
      const feed = await readFile(path.join(root, 'public/rss.xml'), 'utf8')
      expect(feed).toContain('<guid isPermaLink="true">https://384721.xyz/blog/fixture</guid>')
      expect(feed).toContain('<category>Git workflow</category>')
      expect(feed).toContain('&lt;strong&gt;bold&lt;/strong&gt;')
      for (const file of ['src/content/publishedBlogPosts.json', 'public/rss.xml', 'public/blog.md', 'public/llms.txt', 'public/llms-full.txt', 'public/sitemap.xml']) expect(await readFile(path.join(root, file), 'utf8')).not.toMatch(/PRIVATE_DRAFT_SENTINEL|secret-draft/)
